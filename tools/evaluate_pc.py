...
def eval_model(net, batch_size, im_root, im_ann, num_cls):
    ...
    dl = get_data_loader(cfg.im_root, cfg.batch_size, listpath=cfg.train_im_anns, max_iter=cfg.max_iter, mode='val')
    net.eval()
    ...
    mIOU = single_crop(net, dl, num_cls)
    heads.append('single_scale_crop')
    mious.append(mIOU)
    logger.info('single scale crop mIOU is: %s\n', mIOU)

    ms_flip = MscEvalV0((0.5, 0.75, 1, 1.25, 1.5, 1.75), True)
    mIOU = ms_flip(net, dl, num_cls)
    heads.append('ms_flip')
    mious.append(mIOU)
    logger.info('ms flip mIOU is: %s\n', mIOU)

    ms_flip_crop = MscEvalCrop(cropsize=1024, cropstride=2. / 3, flip=True, scales=(0.5, 0.75, 1.0, 1.25, 1.5, 1.75), lb_ignore=255,)
    mIOU = ms_flip_crop(net, dl, num_cls)
...
def evaluate(cfg, weight_pth):
    ...
    ## evaluator
    heads, mious = eval_model(net, 2, cfg.val_im_root, cfg.val_im_anns, num_cls)
